 ...
 * RN-26: No anular afiliación/reingreso si tiene aportes.
 *
 * 6 combinaciones:
 *   AFILIACION × EFECTIVO       → revertir estado + devolver comisión asesor
...
final class InvoiceCancellationService
{
    ...
    /** RN-26: No anular afiliación/reingreso si ya tiene aportes registrados. */
    private function validatePrerequisites(BillInvoice $invoice): void
    {
        // RF-085: bloqueo afiliación y reingreso con aportes
        if (in_array($invoice->tipo, ['AFILIACION', 'REINGRESO'], true) && $invoice->affiliate_id !== null) {
            ...
            if ($hasContributions) {
                throw new InvalidArgumentException(
                    'No se puede anular: el afiliado ya tiene aportes registrados (RN-26).'
                );
            }
        }
    }
    ...
    {
        match ($invoice->tipo) {
            'AFILIACION', 'REINGRESO' => $this->cascadeAffiliation($invoice),
            'APORTE', 'APORTE_INDIVIDUAL' => $this->cascadeContribution($invoice),
            'CUENTA' => $this->cascadeCuentaCobro($invoice),
            default => $this->cascadeGeneric($invoice),
        };
    }

    /** RF-086: Afiliación × {EFECTIVO, CONSIGNACION, CREDITO} */
    private function cascadeAffiliation(BillInvoice $invoice): void
    {
        // Revertir estado afiliado
        ...
        // Devolver comisión asesor si existe
        $this->revertAdvisorCommission($invoice);

        $this->cancelPayments($invoice);
        $this->cancelAccountsReceivable($invoice);

        if ($invoice->payment_method === 'CONSIGNACION') {
            $this->revertBankDeposit($invoice);
        }
    }

    /** RF-086: Aporte × {EFECTIVO, CONSIGNACION} */
    private function cascadeContribution(BillInvoice $invoice): void
    {
        $this->cancelPayments($invoice);
        $this->cancelAccountsReceivable($invoice);
    ...
    /** RF-086: Cuenta × {EFECTIVO, CONSIGNACION} */
    private function cascadeCuentaCobro(BillInvoice $invoice): void
    {
    ...
    private function cascadeGeneric(BillInvoice $invoice): void
    {
        $this->cancelPayments($invoice);
        $this->cancelAccountsReceivable($invoice);
    ...
    /** RF-086: revertir depósito en tp_bank_deposits */
    private function revertBankDeposit(BillInvoice $invoice): void
    {
        DB::table('tp_bank_deposits')
            ->where('invoice_id', $invoice->id)
            ->where('status', '!=', 'ANULADO')
            ->update(['status' => 'ANULADO']);
    }

    /** RF-086: anular comisión asesor vinculada al recibo */
    private function revertAdvisorCommission(BillInvoice $invoice): void
    {
        DB::table('bill_advisor_commissions')
            ->where('affiliate_id', $invoice->affiliate_id)
